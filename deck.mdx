...
> In order to achieve that, **React Testing Library** brings an different approach of testing based on *rendered* components (DOM representation)
 instead of component instances/objects as we usually see in **Enzyme**, for example.
...
we don’t want to know if *addToWishlist* redux action was dispatched or an internal state variable changed. Instead, 
...
> **fireEvent** method might be enough for most cases, but to perform more robust interactions, **user-event** may be a great option to help simulating interactions as much close to real user action as possible.
